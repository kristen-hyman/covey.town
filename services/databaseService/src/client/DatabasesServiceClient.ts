...
export default class DatabaseServiceClient {
  ...
  async getFriends(requestData: UserEmailRequest): Promise<UserInfo[]> {
    const responseWrapper = await this._axios.get<ResponseEnvelope<UserInfo[]>>(
      `/users/${requestData.email}/friends`,
    );
    return DatabaseServiceClient.unwrapOrThrowError(responseWrapper);
  }
  ...
  async getOnlineStatus(requestData: UserEmailRequest): Promise<boolean> {
    const responseWrapper = await this._axios.get<ResponseEnvelope<boolean>>(
      `/users/${requestData.email}/status`,
    );
    return DatabaseServiceClient.unwrapOrThrowError(responseWrapper);
  }
  ...
  async addFriend(requestData: AddFriendRequest): Promise<Record<string, null>> {
    const responseWrapper = await this._axios.post<ResponseEnvelope<Record<string, null>>>(
      `/users/${requestData.email}/friends/${requestData.friendEmail}`,
    );
    return DatabaseServiceClient.unwrapOrThrowError(responseWrapper, true);
  }
